...
import productsData from "../bd-json/labs.json"; // Import the local JSON data
...
const SingleProductPage = async ({ params }: Props) => {
  ...
  const product: ProductType | undefined = productsData.laboratorios.find(
              ...
              <div className="flex items-center">
                {Array.from({ length: 5 }).map((_, index) => (
                  <MdStar
                    key={index}
                    className={`${
                      index < item.rating ? "text-yellow-500" : "text-lightText"
                    }`}
                  />
                ))}
              </div>
